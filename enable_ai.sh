...
MSG_UNLOCK_START=""
MSG_UNLOCK_IPHONE_WARNING_TITLE=""
MSG_UNLOCK_IPHONE_WARNING_1=""
MSG_UNLOCK_IPHONE_WARNING_2=""
MSG_UNLOCK_NOUCHG=""
MSG_UNLOCK_CHMOD=""
MSG_UNLOCK_COMPLETE=""
MSG_SCRIPT_END=""
...
case "$LANG" in
    en*)
        ...
        MSG_MODIFY_PLISTS_COMPLETE="${GREEN}Plist modifications complete.${RESET}"
        ...
        MSG_OPERATION_COMPLETE="${GREEN}Apple Intelligence patch applied successfully.${RESET}"
        MSG_FORCE_US_PROMPT_TITLE="${CYAN}--- Optional Step: Force Location to US ---${RESET}"
        MSG_FORCE_US_PROMPT_BENEFITS="${BOLD}${YELLOW}[Benefits]${RESET} For macOS 26, this can unlock features like ChatGPT, Apple News, and international Apple Maps (requires a suitable network IP)."
        MSG_FORCE_US_PROMPT_WARNING="${BOLD}${YELLOW}[Side Effect]${RESET} This will disable the Gaode version of Apple Maps used in mainland China."
        MSG_FORCE_US_PROMPT_CONFIRM="${BOLD}Would you like to force your system's country code to US? (y/n): ${RESET}"
        ...
        MSG_FORCE_US_SUCCESS="${GREEN}✅ Location successfully forced to US and locked.${RESET}"
        MSG_FORCE_US_NOT_FOUND="${YELLOW}⚠️ Warning: countryd plist not found. Skipping this step.${RESET}"
        ...
        MSG_UNLOCK_IPHONE_WARNING_TITLE="${BOLD}${YELLOW}--- IMPORTANT: iPhone Mirroring Notice ---${RESET}"
        MSG_UNLOCK_IPHONE_WARNING_1="${BOLD}${YELLOW}[Requirement]${RESET} If you use the iPhone Mirroring feature, you must pair your iPhone with this Mac ${BOLD}before${RESET} you change the country code."
        MSG_UNLOCK_IPHONE_WARNING_2="${BOLD}${YELLOW}[Reason]${RESET} Changing the country code may cause a mismatch that can prevent pairing. Pair first, then proceed."
    ...
    *) # Default to Chinese
        ...
        MSG_MODIFY_PLISTS_COMPLETE="${GREEN}属性列表文件修改完成。${RESET}"
        ...
        MSG_OPERATION_COMPLETE="${GREEN}Apple Intelligence 修补程序已成功应用。${RESET}"
        MSG_FORCE_US_PROMPT_TITLE="${CYAN}--- 可选步骤：强制修改地区为美国 ---${RESET}"
        MSG_FORCE_US_PROMPT_BENEFITS="${BOLD}${YELLOW}[好处]${RESET} 在 macOS 26 系统中，此操作可解锁 ChatGPT、Apple News 及国际版苹果地图等功能（需配合相应的网络IP）。"
        MSG_FORCE_US_PROMPT_WARNING="${BOLD}${YELLOW}[副作用]${RESET} 此操作将导致无法在中国大陆境内使用高德版苹果地图。"
        MSG_FORCE_US_PROMPT_CONFIRM="${BOLD}您是否希望将系统的国家代码强制修改为美国 (US)？(y/n): ${RESET}"
        ...
        MSG_FORCE_US_SUCCESS="${GREEN}✅ 系统地区已成功强制修改为美国并已锁定。${RESET}"
        MSG_FORCE_US_NOT_FOUND="${YELLOW}⚠️ 警告：未找到 countryd 数据库文件，跳过此步骤。${RESET}"
        ...
        MSG_UNLOCK_IPHONE_WARNING_TITLE="${BOLD}${YELLOW}--- 重要：关于 iPhone 镜像功能的提醒 ---${RESET}"
        MSG_UNLOCK_IPHONE_WARNING_1="${BOLD}${YELLOW}[要求]${RESET} 如果您使用 iPhone 镜像功能，必须在修改国家代码前，先完成 iPhone 与这台 Mac 的配对。"
        MSG_UNLOCK_IPHONE_WARNING_2="${BOLD}${YELLOW}[原因]${RESET} 修改国家代码可能导致 macOS 和 iPhone 的代码不匹配，从而阻止配对。请务必先配对，再继续。"
...
# ... (The rest of the script functions remain the same)
# ── File Paths & Dirs
...
force_us_location() {
    echo -e "$MSG_FORCE_US_START"
    if [ ! -f "$COUNTRYD_PLIST" ]; then
        echo -e "$MSG_FORCE_US_NOT_FOUND"
        return
    fi
    echo -e "$MSG_FORCE_US_UNLOCKING"
    ...
    echo -e "$MSG_FORCE_US_BACKUP"
    sudo plutil -convert xml1 "$COUNTRYD_PLIST"
    echo -e "$MSG_FORCE_US_CONVERT_XML"
    ...
    echo -e "$MSG_FORCE_US_REPLACE"
    sudo plutil -convert binary1 "$COUNTRYD_PLIST"
    echo -e "$MSG_FORCE_US_CONVERT_BIN"
    echo -e "$MSG_FORCE_US_LOCKING"
    sudo chmod 444 "$COUNTRYD_PLIST"
    sudo chflags uchg "$COUNTRYD_PLIST"
    echo -e "$MSG_FORCE_US_LOCK_COMPLETE"
    echo -e "$MSG_FORCE_US_SUCCESS"
}
...
enable_ai() {
    ...
    echo ""
    echo -e "${BLUE}==============================================${RESET}"
    echo -e "$MSG_START_STEPS"
    echo -e "$MSG_UNLOCK_PERMS"
    ...
    echo -e "$MSG_MODIFY_PLISTS_START"
    ...
    echo -e "$MSG_MODIFY_PLISTS_COMPLETE"
    ...
    echo ""
    echo -e "${BLUE}==============================================${RESET}"
    echo -e "$MSG_OPERATION_COMPLETE"
    
    # 7. Optional: Force location to US, now with iPhone Mirroring warning
    echo ""
    echo -e "$MSG_FORCE_US_PROMPT_TITLE"
    echo -e "$MSG_FORCE_US_PROMPT_BENEFITS"
    echo -e "$MSG_FORCE_US_PROMPT_WARNING"
    echo ""
    echo -e "$MSG_UNLOCK_IPHONE_WARNING_TITLE"
    echo -e "$MSG_UNLOCK_IPHONE_WARNING_1"
    echo -e "$MSG_UNLOCK_IPHONE_WARNING_2"
    echo -e "${BLUE}----------------------------------------------------${RESET}"
    read -r -p "$(echo -e "$MSG_FORCE_US_PROMPT_CONFIRM")" force_us < /dev/tty
    if [[ "$force_us" =~ ^[Yy]$ ]]; then
        force_us_location
    fi
